...
def build_connections():
    ...
    WALKING_DISTANCE = 500  # Increased from 500 meters
...
def format_route(path):
    ...
    for i, node in enumerate(path):
        if node.startswith('walk|'):
            ...
            if not current_segment or current_segment['line'] != cleaned_line or current_segment['type'] != line_type:
                append_valid_segment(current_segment)
                current_segment = {
                    'type': line_type, 'line': cleaned_line,
    ...
    for i in range(1, len(segments)):
        ...
        if prev_segment['type'] == 'walk' and current_segment['type'] == 'walk':
            # Merge current walk segment into the previous one
            prev_segment['to'] = current_segment['to']
            prev_segment['distance'] += current_segment['distance']
            # Append coordinates, avoiding duplicates
    ...
    # Pass 3: Calculate final durations
    for seg in merged_segments:
        if seg['type'] == 'walk':
            # Walking speed approx 1.3 m/s
            seg['duration'] = seg['distance'] / 1.3
        else:
            # 2 minutes per station stop
